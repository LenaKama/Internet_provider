...
public class OpeningBalanceDAO {
    ...
    private final static String SELECT_LAST_MONTH_BALANCE = "SELECT ob_sum FROM opening_balance WHERE MONTH(ob_date) = MONTH(DATE_SUB(CURDATE(), INTERVAL 3 DAY)) AND YEAR(ob_date) = YEAR(DATE_SUB(CURDATE(), INTERVAL 3 DAY)) AND us_login = ?";
